...
public class ContactUs extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
					...
					if (source.length() < 10) {
						...
						if (!Character.isDigit(source.charAt(0)))
							...
							if (dest.toString().length() > 13) {
								...
								if (source.toString().equals("7")
										|| source.toString().equals("8")
										|| source.toString().equals("9")) {
									return "(" + source;
								} else {
									return "";
								}
		...
		mob.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				/*
				 * System.out.println("sting value"+str); int
				 * edittextstr=str.length(); str.substring(0,13);
				 * System.out.println(str); System.out.println(edittextstr);
				 * mob.setText(str);
				 */
			...
			public void afterTextChanged(Editable s) {
				 ...
				 * System.out.println("sting value"); String
				 * subi=mob.getText().toString(); subi=subi.substring(0,13);
				 * System.out.println("sting value"+subi); //
				 * mob.setText(mob.getText().toString().subSequence(0, 13));
				 * mob.setText(subi); mob.setSelection(mob.getText().length());
				 * // mob.setText("Just 10 Number"); }else{
				 * 
				 * }
				 */
			}
			...
			public void onClick(View view) {
				...
				if (isInternetPresent) {
					...
					if (fstname.length() > 0 && lstname.length() > 0
							&& email1.length() > 0 && organistn.length() > 0
							&& mob.length() > 0 && add1.length() > 0
							&& city1.length() > 0 && state1.length() > 0) {
						a = 1;

						{
							if (firstname.length() > 3
									...
									if (lastname.length() > 3
											&& isValidName(lastname)) {
										{
											if (isValidEmail(email)) {
													...
													if (isValidOther1(organisation)) {

														{
															if (isValidNumber(mobile)) {
																			...
																			if (isValidOther1(city)) {
																				{
																					if (isValidOther1(state)) {
																						...
																						// Message
																						alertDialog
																								.setMessage("State should contain only alphabets.");
																				...
																				// Message
																				alertDialog
																						.setMessage("City should contain only alphabets.");
																...
																// Message
																alertDialog
																		.setMessage("Mobile number should contain only numbers.");
														...
														alertDialog
																.setTitle("Invalid organization name");

														// Setting Dialog
														// Message
														alertDialog
																.setMessage("Organization name should contain only alphabets.");
												...
												alertDialog
														.setMessage("Email should contain alphabets,numbers,@ . _");
										...
										// Setting Dialog Message
										alertDialog
												.setMessage("Lastname should contain only alphabets,4-16 characters.");
								...
								// Setting Dialog Message
								alertDialog
										.setMessage("Firstname should contain only alphabets,4-16 characters.");
			...
			try {
...

				{
					...
					messageall = String.format("Hi " + "\n\n"
							...
							+ "%s\n\n" + "City : " + "%s\n\n" + "State : "
							+ "%s" + "\n\n" + "\n\n", firstname, lastname,
							email, organisation, mobile, address1, city, state);
					System.out.println(messageall);

					sender.sendMail(part_name, messageall, "deemgps@gmail.com",
							"sales@deemsysinc.com");
					sender.sendMail(part_name1, message, "deemgps@gmail.com",
							email);
				...
				try {
					// sender.sendMail("GPS Report",messageall,
					// "imans.vijay@gmail.com",sender_mail);
	...
	class AttemptLogin extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			paramsE.add(new BasicNameValuePair("organisation",
					ContactUs.organisation));
